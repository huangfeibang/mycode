 ...
 * @date 2015.02
 *
 * @brief 1.add smartlink function
 *        2.add get ip status
 *        3.update get version function
 *        4.add baud change function
 * @author Alex
 * @date 2016.01
...
class ESP8266 {
 public:
     ...
     * Change baud. 
     *
     * after Set the command,MCU band need changed. 
     *
     * @retval true - success.
     * @retval false - failure.
     */
     ...
     * start smartLink , 30sec timeout. 
     *
	 * @param link_msg - SSID and pwd
     * @retval true - success.
     * @retval false - failure.
     */	
	bool smartLink(uint8_t  type,String &link_msg);
...
*/
    ...
    /**add by LC 2016.01.05 16:28
     * Get system status
     *
     * @return status. -1 获取失败
     */	
 ...
 private:
	...
	//add by LC 2016.01.05 14:19
    bool eATCWSMARTSTART(uint8_t type,String &link_msg);
	bool eATCWSMARTSTOP(void);
	bool eATUART(uint32_t baud);
    /*
     * +IPD,len:data
     * +IPD,id,len:data
     */
